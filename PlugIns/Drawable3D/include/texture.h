...
//extern unsigned  char* LoadBitmap(char *filename);

int num_texture=-1;

inline unsigned char* LoadBitmap(const char *filename) 
{
    ...
    if( (l_file = fopen(filename, "rb"))==NULL) return (NULL); // Open the file for reading
...
inline int LoadBitmapTexture(char *filename) 
{
	 int i, j=0; //Index variables
    FILE *l_file; //File pointer
    unsigned char *l_texture; //The pointer to the memory zone in which we will load the texture
