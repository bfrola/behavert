...
static const int GRFSIZE = GRFELEMENTS * sizeof(BehaveRT::uint); 
...
static const int PREFIX_GRFSIZE = PREFIX_GRFELEMENTS * sizeof(BehaveRT::uint);
...
static const int SHUFFLE_GRFSIZE = SHUFFLE_GRFELEMENTS * sizeof(BehaveRT::uint); 


#define SDATA( index)      CUT_BANK_CHECKER(sdata, index)

// Prefix sum data
BehaveRT::uint gRadixSum[TOTALRADIXGROUPS * RADICES];
__device__ BehaveRT::uint dRadixSum[TOTALRADIXGROUPS * RADICES];

BehaveRT::uint gRadixBlockSum[PREFIX_NUM_BLOCKS];
//__device__ BehaveRT::uint dRadixBlockSum[PREFIX_NUM_BLOCKS];
__device__ BehaveRT::uint dRadixBlockSum[TOTALRADIXGROUPS * RADICES];
// STRANGE!: works with dRadixBlockSum[TOTALRADIXGROUPS * RADICES] 
//		instead of dRadixBlockSum[PREFIX_NUM_BLOCKS] 


extern __shared__ BehaveRT::uint sRadixSum[];
...
//!                  byte
////////////////////////////////////////////////////////////////////////////////
__global__ void RadixSum(KeyValuePair *pData, BehaveRT::uint elements, BehaveRT::uint elements_rounded_to_3072, BehaveRT::uint shift)
{
    BehaveRT::uint pos    = threadIdx.x;
    ...
    BehaveRT::uint tmod   =   threadIdx.x % RADIXTHREADS;
    BehaveRT::uint tpos   =   threadIdx.x / RADIXTHREADS;
    ...
    BehaveRT::uint element_fraction  =   elements_rounded_to_3072 / TOTALRADIXGROUPS;
    ...
    BehaveRT::uint end  = pos + element_fraction;
    pos      += tmod; 
    //printf("pos: %d\n", pos);
    __syncthreads();
    ...
    while (pos < end )
    {
        ...
        BehaveRT::uint p = ((key >> shift) & RADIXMASK) * RADIXGROUPS;
    ...
    BehaveRT::uint offset = blockIdx.x * RADIXGROUPS;
    BehaveRT::uint row    = threadIdx.x / RADIXGROUPS;
    BehaveRT::uint column = threadIdx.x % RADIXGROUPS;
    while (row < RADICES)
    {
...
__global__ void RadixPrefixSum()
{
    ...
    BehaveRT::uint brow       = blockIdx.x * (RADICES / PREFIX_NUM_BLOCKS);
    BehaveRT::uint drow       = threadIdx.x / TOTALRADIXGROUPS; // In default parameterisation this is always 0
    BehaveRT::uint dcolumn    = threadIdx.x % TOTALRADIXGROUPS; // And similarly this is always the same as threadIdx.x   
    BehaveRT::uint dpos       = (brow + drow) * TOTALRADIXGROUPS + dcolumn;
    BehaveRT::uint end        = ((blockIdx.x + 1) * (RADICES / PREFIX_NUM_BLOCKS)) * TOTALRADIXGROUPS;
    // Shared mem addressing
    BehaveRT::uint srow       = threadIdx.x / (PREFIX_BLOCKSIZE / PREFIX_NUM_THREADS_PER_BLOCK);
    BehaveRT::uint scolumn    = threadIdx.x % (PREFIX_BLOCKSIZE / PREFIX_NUM_THREADS_PER_BLOCK);
    BehaveRT::uint spos       = srow * (PREFIX_BLOCKSIZE / PREFIX_NUM_THREADS_PER_BLOCK + 1) + scolumn;
    ...
    {
        int p  = pos - m;
        BehaveRT::uint t = ((p > 0) ? sRadixSum[p] : 0);
        __syncthreads();
        sRadixSum[pos] += t;
        __syncthreads();
        m *= 2;
...
//!                  byte
////////////////////////////////////////////////////////////////////////////////
__global__ void RadixAddOffsetsAndShuffle(KeyValuePair* pSrc, KeyValuePair* pDst, BehaveRT::uint elements, BehaveRT::uint elements_rounded_to_3072, int shift)
{
    ...
    {
        int ppos = pos - n;
        BehaveRT::uint t0 = ((pos < PREFIX_NUM_BLOCKS) && (ppos >= 0)) ? sRadixSum[SHUFFLE_GRFOFFSET + ppos] : 0;
        __syncthreads();
    ...
    pos       = (blockIdx.x * RADIXGROUPS + tpos) * element_fraction;
    BehaveRT::uint end  = pos + element_fraction; //(blockIdx.x * RADIXGROUPS + tpos + 1) * element_fraction;
    pos      += tmod; 
  ...
  //          kvp = pSrc[pos];
        ...
        // Calculate position of radix counter to increment
        BehaveRT::uint p = ((kvp.key >> shift) & RADIXMASK) * RADIXGROUPS;
